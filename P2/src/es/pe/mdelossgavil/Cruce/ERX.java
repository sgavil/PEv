...
public class ERX implements ICruce {
	...
	public void reproduccion(ACromosoma p1, ACromosoma p2, ACromosoma h1, ACromosoma h2) {
		...
		for (int i = 0; i < p1.get_longitud(); i++) {
			...
			// Por la izquierda
			if (i - 1 < 0)
				matriz.get((int) p1.getCodificacion().get(i))
						.add((int) p1.getCodificacion().get(p1.get_longitud() - 1));
			else
				matriz.get((int) p1.getCodificacion().get(i)).add((int) p1.getCodificacion().get(i - 1));
		}
		...
		for (int i = 0; i < p2.get_longitud(); i++) {
			...
			// Por la izquierda
			if (i - 1 < 0) {
				if (!matriz.get((int) p2.getCodificacion().get(i))
						.contains((int) p2.getCodificacion().get(p2.get_longitud() - 1)))
					matriz.get((int) p2.getCodificacion().get(i))
							.add((int) p2.getCodificacion().get(p2.get_longitud() - 1));
	...
	private void Cruce(ACromosoma padre1, ACromosoma padre2, ACromosoma hijo1, ACromosoma hijo2,
		...
		while (index < hijo1.getCodificacion().size()) {
			...
			if (matriz.get(indice).size() > 0) {
				...
				// Tenemos todos los adyacentes de ese numero ya metidos, por tanto tenemos que
				// volver atras
