...
public class Aritmetico implements ICruce {
	...
	public void reproduccion(ACromosoma p1, ACromosoma p2, ACromosoma h1, ACromosoma h2) {
		/*
		 * int tam_pob = poblacion.size(); int seleccionCruce[] = new int[tam_pob];
		 * 
		 * // Contador seleccionados int num_sele_cruce = 0; int punto_cruce; float
		 * prob;
		 * 
		 * ACromosoma hijo1 = null, hijo2 = null;
		 * 
		 * /* Cogemos el tipo de Cromosoma del problema if (problema == "P2") { hijo1 =
		 * new CromosomaP2(); hijo2 = new CromosomaP2(); }
		 * 
		 * hijo1.inicializa_cromosoma(); hijo2.inicializa_cromosoma();
		 * 
		 * for (int i = 0; i < tam_pob; i++) { // Creamos una prob aleatoria entre [0,1)
		 * prob = (float) Math.random();
		 * 
		 * // En caso de que sea menos a la prob de Cruce // se escoge a ese invididuo
		 * para cruzarse if (prob < probCruce) { seleccionCruce[num_sele_cruce] = i;
		 * num_sele_cruce++; } }
		 * 
		 * // El numero de seleccionados se hace par if (num_sele_cruce % 2 == 1)
		 * num_sele_cruce--;
		 * 
		 * for (int i = 0; i < num_sele_cruce; i += 2) { ACromosoma padre1 =
		 * poblacion.get(seleccionCruce[i]); ACromosoma padre2 =
		 * poblacion.get(seleccionCruce[i + 1]); Cruce(padre1, padre2, hijo1, hijo2);
		 * 
		 * if (problema == "P2") { poblacion.set(seleccionCruce[i], new
		 * CromosomaP2(hijo1)); poblacion.set(seleccionCruce[i + 1], new
		 * CromosomaP2(hijo2)); } }
		 */
